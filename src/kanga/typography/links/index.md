...
For more fine-grained control over links, use the Sass mixin.

``` {.language-scss}
@include type-style-link($color, $focus, $hover, $underline);
```
...
| `$underline` | Should the default state of this link have an underline? Either `underline` or `none`. The default is `underline`. |
